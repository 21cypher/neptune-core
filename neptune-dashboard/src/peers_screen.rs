...
pub struct PeersScreen {
    ...
    scrollable_table: ScrollableTable<PeerInfo>,
...
impl PeersScreen {
    pub fn new(
        ...
        Self {
            ...
            scrollable_table: ScrollableTable::new(data.clone()),
        }
    }
    ...
    pub fn clear_selected_peer_standing(&self) {
        let Some(selected_index) = self.scrollable_table.selected_data_index() else {
            return;
        };

        let peer_ip = {
            let items = self.scrollable_table.data().lock().unwrap();
            items.get(selected_index).and_then(|peer| {
